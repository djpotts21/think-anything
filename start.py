...
def accept_friend():
    ...
    flash("Removed pending request for " + username)
...
def contact():
    ...
    if request.method == "POST":
        ...
        # insert message
        mongo.db.admin_messages.insert_one(message)
        # flash message
...
# report page
@app.route("/report-it", methods=["GET", "POST"])
def report_it():
    """Report page"""
    if request.method == "POST":
        # build message data
        ...
        mongo.db.admin_messages.insert_one(message)
        # flash message
        flash("Your report has been sent to the admin")
        # return to contact page
        return redirect(url_for("contact"))
    ...
    if session.get("logged-in") == "yes":
        ...
        user_data = mongo.db.users.find_one({"username": session["user"]})
        # return contact page with parameters
        return render_template("report_it.html",
                               background=background,
                               user_data=user_data)
    else:
        # return contact page
        return render_template("report_it.html", background=background)
